...
proc ReadBarcode {} {
  ...
  if {[info exists gaSet(1.barcode1)] && $gaSet(1.barcode1)!="skipped"} {
    puts "[MyTime]barcode exist: $gaSet(1.barcode1) , no need read it "
     return 0
  }
  ...
  while {$ret != "0" } {
    ...
    if { $ret == "-3" || $ret == "-2" ||  $ret == "-1" } {
      ...
      if { $ret == "-3"} {
        AddToPairLog $gaSet(pair) "Read barcode skipped"
      } else {
        AddToPairLog $gaSet(pair) "Read barcode fail"
      }
      return $ret
    }
	}	
