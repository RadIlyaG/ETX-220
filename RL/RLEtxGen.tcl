...
proc PortsConfig {ip_ID args } {
  ...
  if {$vaEtxGenStatuses($ip_ID,package) == "RLPlink"} {
	...
	set portsList $vaEtxGenStatuses($ip_ID,phyPorts)
	#processing command line parameters
  foreach {param val}   $args   {
                      ...
                      if {$val == "?"} {
                        return "updated port options:  1-4/all.........."
											...
											if {[regexp -nocase {all} $val]} {
  											    set updGen [string tolower $val]
											} elseif {$val == 1 || $val == 2} {
  											  set updGen $val
											} elseif {($val == 3 || $val == 4) && $portsList == "1 2"} {
                  		    set	gMessage "PortsConfig procedure: The value $val of parameter $param wrong\nThis Etx204 have only 2 ports"
                          #return [RLEH::Handle SAsyntax gMessage]
													return $fail
											} elseif {$val == 3 || $val == 4} {
	...
	#if generator before this procedure was failed and wait for save command, i release it from this state.
	SendToEtxGen $ip_ID "ns\r" "ETX-204A-" 4
	set ret 0
  ...
  foreach {param val}   $args   {
    ...
    switch -exact -- $param  {
                      ...
                      if {$val == "?"} {
						...
						-ethType {
										  if {[regexp -nocase {[0-9a-f]{4}} $val] == 0} {
     ...
     if {![regexp "$param\[^\n\]+" $gEtxGenBuffer(id$ip_ID) match]} {
        ...
        return $fail
		 ...
		 foreach {gener} {1 2 3 4} {
			 ...
			 } elseif {[string match -nocase "*n/a*" $results(id$ip_ID,$param,Gen$gener)]} {
			     set results(id$ip_ID,$param,Gen$gener) "n/a"
			 } else {
...
proc OpenEtxGen {ip_address ip_connection ip_package ip_place} {
  ...
  DelayMs 20
  set ret [SendToEtxGen $ip_place "4\r" "MaxAdvSpeed" 2]
	if {[set ind [lsearch $gEtxGenBuffer(id$ip_place) "Autonegotiat"]] == -1} {
    switch -exact -- $ip_connection  {
    ...
    set gMessage "OpenEtxGen:  Can't find  Autonegotiat parameter in 'Current configured param' menu"
    return $fail
	}
	#puts 4
	if {[lrange $gEtxGenBuffer(id$ip_place) [expr $ind + 3] [expr $ind + 4]] == "n/a n/a"} {
	  #puts 5
    set  vaEtxGenStatuses($ip_place,phyPorts) "1 2"
	} else {
     set  vaEtxGenStatuses($ip_place,phyPorts) "1 2 5 6"
		# puts 6
	}
  set ret [expr [SendToEtxGen $ip_place "!" "Kernel Objects" 4] | $ret]
  DelayMs 20
...
#	set vaEtxGenSet(PacketType) MAC
  ...
  #wm resizable $base 0 0
	...
	if {$vaEtxGenSet(starpack)} {
    wm geometry $base 900x680
	} else {
      wm geometry $base 900x660
	}
		  ...
		  set bbox [ButtonBox [$vaEtxGenGui(genstat,tools) getframe].bbox2 -spacing 0 -padx 1 -pady 1]
		    ...
		    set vaEtxGenGui(genstat,tools,clearone) [$bbox add -image [Bitmap::get $vaEtxGenSet(rundir)/Images/clearone] \
		      ...
		      -helptext "Clear current port statistics" -command {RLEtxGen::ClearEtxGenStatistics}]
				grid $vaEtxGenGui(genstat,tools,clear) $vaEtxGenGui(genstat,tools,clearone) -padx 10
			pack $bbox -side left
